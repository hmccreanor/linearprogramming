...
class Tableu():
    ...
    def __init__(self, inFile):
        ...
        with open(inFile, "r") as f:
            ...
            self.tableu = np.zeros((n_expressions + 1, n_variables + n_slack + 2), dtype=np.intc)

            # Update the rows regarding the linear expressions
            for i in range(n_expressions):
                count = 0 # Storing the row index 
                for variable in variables:
                    self.tableu[i][count] = coefficient_table[i][variable]
                    count += 1
                self.tableu[i][count + i] = 1 # Set the respective slack variable coefficient to be 1
                self.tableu[i][n_variables + n_slack + 1] = constraints[i] # Set the last element in the row to be the constraint for that particular linear expression

            # Update the last row
            for i in range(n_variables):
                variable = variables[i]
                self.tableu[n_expressions][i] = -1 * objective_coefficients[variable]

            self.tableu[n_expressions][n_variables + n_slack] = 1 # Set "score" variable coefficient to 1
            self.slack_start_index = n_variables
